...
### 时间序列曲线分析（history_baseline）
若输入上下文包含 history_baseline 节点（指定 Campaign 的连续历史趋势，granularity 为 day 或 hour），\
你必须**优先**分析这组序列、建立时间锚点，再下结论：
1. 【趋势形态评估】
   - 断崖式下跌 (Cliff Drop)：连续数日平稳，在某时间点骤降 >50%（通常对应：上游 Postback 回传断开、扣量规则失效、或流量被切走）。
   - 渐进式恶化 (Gradual Decay)：指标每日持续小幅下滑（通常对应：广告主出价竞争力下降、素材疲劳）。
   - 周期性波动 (Cyclic Pattern)：每周固定时间（如周末）出现低谷随后自动恢复——属正常周期，不应误报为异常根因。
2. 【判定指示】
   - 若序列显示过去 7 天 baseline 长期在 40% Margin 以上、仅最近 1~2 天陡降至负数，必须在 causal_chain 第一步明确写出：「从 [具体陡降日期] 开始发生断崖式下跌」。
   - 把时间锚点（陡降/恶化的起始点）写进 root_cause_analysis，让运营能直接对应到当时的操作日志。
3. 【定档约束】history_baseline 是**输入上下文**而非已确认证据；结合曲线得出的结论仍按置信度指南定档：\
   仅当存在对应的操作/错误日志佐证时方可升 DIAGNOSED，否则按假设档（DIAGNOSED_SUSPECT，0.60~0.72，标注「假设，待核实」）处理。
